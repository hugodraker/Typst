...
#define MAX_RUNS         256
#define STREAM_BUF_SIZE  1048576 // 1 MB initial stream buffer
...
/* Dynamic In-Memory Stream Buffer */
typedef struct {
    char* data;
    size_t len;
    size_t cap;
...
static double page_width = 612.0;   // US-Letter standard width
static double page_height = 792.0;  // US-Letter standard height
static double margin_top = 70.86;   // 2.5 cm
static double margin_bottom = 70.86;// 2.5 cm
static double margin_left = 56.69;  // 2.0 cm
static double margin_right = 56.69; // 2.0 cm
...
static void sb_printf(StreamBuffer* sb, const char* fmt, ...) {
    ...
    while (sb->len + needed + 1 >= sb->cap) {
        ...
        if (!new_data) {
            fprintf(stderr, "Fatal: Out of memory expanding PDF stream buffer.\n");
            return;
        }
        sb->data = new_data;
    }
...
Node* alloc_node(NodeType type) {
    if (node_count >= MAX_NODES) {
        fprintf(stderr, "Warning: Node buffer capacity (%d) reached.\n", MAX_NODES);
        return NULL;
    }
...
static int skip_whitespace_and_comments(void) {
    while (source[source_pos]) {
        if (isspace((unsigned char)source[source_pos])) {
            source_pos++; continue;
        }
    ...
    *run_count = 0;
    ...
    while (*p) {
        ...
        if (strncmp(p, "#text(", 6) == 0) {
            ...
            if (extract_param_value(params, "size", val, sizeof(val))) local_state.font_size = parse_size(val);
...
while (*p == ' ' || *p == '\t') p++;
            if (*p == '[') {
                ...
                /* --- Replace the local stack array with a heap allocation --- */
                ...
                if (sub_runs) {
                    ...
                    free(sub_runs); // Clean up the heap
                }
            }
            continue;
        }
...
static void parse_set_directive(void) {
    ...
    if (match_str("#set text(")) {
        ...
        char val[128];
        if (extract_param_value(params, "size", val, sizeof(val))) current_state.font_size = parse_size(val);
...
static Node* parse_table_or_grid(NodeType type) {
    ...
    if (!t) return NULL;
    if (type == NODE_TABLE) match_str("#table"); else match_str("#grid"); // Relaxed matching
    ...
    if (source[source_pos] == '(') {
        ...
        source_pos++; // Consume '('
        while (source[source_pos] && depth > 0) {
            /* FIX: Break if we reach the start of cells BEFORE changing depth */
    ...
    if (extract_param_value(param, "columns", cols_str, sizeof(cols_str))) {
        ...
        t->cell_cols = 1; t->is_fr[0] = 1; t->col_widths[0] = 1.0;
    }
    ...
    if (extract_param_value(param, "fill", fill_val, sizeof(fill_val))) {
        if (strstr(fill_val, "=>")) {
            ...
            t->fill_color = get_color(fill_val);
        }
    }
    ...
    while (source[source_pos]) {
        ...
        if (source[source_pos] == ',') { consume(); continue; }
        ...
        if (strncmp(&source[source_pos], "align(", 6) == 0) {
            match_str("align(");
        ...
        if (source[source_pos] == '[') {
            extract_bracket_content(cell_node->content, MAX_STR_LEN);
        } else {
            int ci = 0;
            while (source[source_pos] && source[source_pos] != ',' && source[source_pos] != ')' && source[source_pos] != ']' && ci < MAX_STR_LEN - 1) {
                cell_node->content[ci++] = source[source_pos++];
            }
            cell_node->content[ci] = '\0';
        }

        if (row < MAX_GRID_ROWS && col < MAX_TABLE_COLS) {
            t->cells[row][col] = cell_node;
        }
        cell++;
    }
...
static Node* parse_block_node(NodeType type) {
    ...
    if (!b) return NULL;
    match_str(type == NODE_RECT ? "#rect" : "#block"); // Relaxed matching
    ...
    skip_whitespace_and_comments();
    if (source[source_pos] == '[') {
        extract_bracket_content(b->content, MAX_STR_LEN);
    }
    return b;
}
...
static Node* parse_plain_text(void) {
    ...
    int i = 0;
    while (source[source_pos] && source[source_pos] != '\n' && i < MAX_STR_LEN - 1) {
        if (source[source_pos] == '#' && !(i > 0 && p->content[i-1] == '\\')) {
            if (i > 0) break;
            source_pos++;
            ...
            if (source[source_pos] == '(') {
                int depth = 1;
                source_pos++;
                while (source[source_pos] && depth > 0 && source[source_pos] != '\n') {
                    if (source[source_pos] == '(') depth++;
                    else if (source[source_pos] == ')') depth--;
                    source_pos++;
                }
...
static Node* parse_element(void) {
    ...
    if (strncmp(&source[source_pos], "#set", 4) == 0) {
        parse_set_directive();
        return NULL;
    }
...
// Replace the #v, #line, #rect, #grid, #table matching block in parse_element() with this:
    ...
    if (strncmp(&source[source_pos], "#line", 5) == 0) return parse_line();
    if (strncmp(&source[source_pos], "#rect", 5) == 0) return parse_block_node(NODE_RECT);
    if (strncmp(&source[source_pos], "#grid", 5) == 0) return parse_table_or_grid(NODE_GRID);
    if (strncmp(&source[source_pos], "#table", 6) == 0) return parse_table_or_grid(NODE_TABLE);

    /* Top-level #align(...) [...], #text(...) [...], or #pad(...) [...] wrapper */
    if (strncmp(&source[source_pos], "#align(", 7) == 0 ||
        strncmp(&source[source_pos], "#text(", 6) == 0 ||
        strncmp(&source[source_pos], "#pad(", 5) == 0) {
        Node* p = alloc_node(NODE_PARAGRAPH);
        if (!p) return NULL;
        if (strncmp(&source[source_pos], "#align(", 7) == 0) {
...
static Node* parse_document(void) {
    ...
    while (skip_whitespace_and_comments() != 0) {
        ...
        if (elem && doc->child_count < 512) {
            doc->children[doc->child_count++] = elem;
        }
        if (source_pos == prev_pos) {
            source_pos++;
        }
        if (source[source_pos] == '\n') source_pos++;
    }
...
static double render_styled_text(StreamBuffer* sb, const char* text, double x, double y,
    ...
    double current_y = y - font_size;
    double start_x = x;
    double draw_x = start_x;
    double max_h_in_line = font_size;

    for (int i = 0; i < run_count; i++) {
        if (strcmp(runs[i].text, "\n") == 0) {
            current_y -= (max_h_in_line + 4.0);
            draw_x = start_x;
            max_h_in_line = font_size;
            continue;
        }

        /* FIX: Implement manual space separation and boundary checks for Word Wrap */
        char word[1024];
        int wi = 0;
        ...
        while (*p) {
            if (*p == ' ' || *p == '\t') {
                word[wi] = '\0';
                if (wi > 0) {
                    double word_w = wi * runs[i].font_size * 0.52;
                    if (draw_x + word_w > start_x + max_w && draw_x > start_x) {
                        current_y -= (max_h_in_line + 4.0);
                        draw_x = start_x;
                    }
                    if (!dry_run && sb) {
                        pdf_draw_text_run(sb, word, draw_x, current_y, runs[i].font_size, runs[i].is_bold, runs[i].is_italic, runs[i].color);
                    }
                    draw_x += word_w;
                    wi = 0;
                }
                draw_x += runs[i].font_size * 0.3; // Append the space width
                p++;
            } else {
        ...
        if (wi > 0) { // Render the final trailing word fragment
            word[wi] = '\0';
            double word_w = wi * runs[i].font_size * 0.52;
            if (draw_x + word_w > start_x + max_w && draw_x > start_x) {
                current_y -= (max_h_in_line + 4.0);
        ...
        max_h_in_line = MAX(max_h_in_line, runs[i].font_size);
    }

    double final_y_diff = (y - current_y) + 4.0;
    free(runs);
    return final_y_diff;
}
...
static double measure_node_height(Node* n, double max_w) {
    ...
    switch (n->type) {
        ...
        case NODE_RECT: {
            ...
            double ch = render_styled_text(NULL, n->content, 0, 0, rw - n->inset*2, n->font_size, 0, current_state.fill_color, 1);
            return ch + n->inset * 2.0;
        }
        default: return 16.0;
    }
}
...
static void render_node(StreamBuffer* sb, Node* n, double* y, double max_w, double start_x, int page_idx) {
    ...
    /* Auto-Pagination Guard: Prevent drawing off the bottom margin */
    ...
    switch (n->type) {
        ...
        case NODE_RECT: {
            ...
            double content_h = render_styled_text(NULL, n->content, 0, 0, render_w - inset*2, n->font_size, 0, current_state.fill_color, 1);
        ...
        case NODE_GRID: {
            ...
            for (int r = 0; r < n->cell_rows; r++) {
                ...
                for (int c = 0; c < n->cell_cols; c++) {
                    ...
                    if (cell) {
                        double ch = render_styled_text(NULL, cell->content, 0, 0, actual_widths[c], n->font_size, cell->align, current_state.fill_color, 1);
        ...
        case NODE_TABLE: {
            ...
            for (int r = 0; r < n->cell_rows; r++) {
                ...
                for (int c = 0; c < n->cell_cols; c++) {
                    Node* cell = n->cells[r][c];
                    if (cell) {
                        double ch = render_styled_text(NULL, cell->content, 0, 0, actual_widths[c] - inset*2, n->font_size, cell->align, current_state.fill_color, 1);
                        max_row_h = MAX(max_row_h, ch);
                    }
                }
...
int export_pdf(Node* root, const char* filename) {
    ...
    if (sb.len > 0 && sb.data) {
        ...
        if (written != sb.len) {
            fprintf(stderr, "Error: Failed to write entire PDF stream buffer to file.\n");
        }
    }
    fprintf(f, "\nendstream\nendobj\n");
...
int main(int argc, char** argv) {
    ...
    if (!fp) {
        fprintf(stderr, "Error: Cannot open input file '%s'. Please check that the file exists.\n", argv[1]);
        return 1;
    }
